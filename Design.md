...
| `MuleDetailDrawer`      | `sm`                                     | Sheet width: full viewport (`<sm`) ↔ 640px (`sm+`); top-right `Close` icon hidden at `sm+`                |
...
Inside the meta block: display-serif name, `Lv.NN` accent-numeric + mono-spaced class label + weekly-income badge (`--surface-2` background, tiny eyebrow + mono number + italic "mesos"), an **Active Toggle** (pill button — green dot + label "ACTIVE" / muted "INACTIVE"), and the **CrystalTally** (see below). Top-right: ghost-icon **Trash** (morphs into inline "Delete? Yes / Cancel" confirm bar, destructive-tinted) and a **Close** icon (`sm:hidden` — desktop dismisses via the sheet backdrop).
