  ...
  return (
    ...
    >
      <Select.Trigger
        ...
        className="inline-flex h-[32px] items-center gap-1.5 rounded-[8px] border border-border bg-(--surface) px-3 text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground cursor-pointer transition-colors hover:border-(--panel-border-hover)"
      >
        {world ? (
          <span className="relative top-px">{world.label}</span>
        ) : (
          <span className="relative top-px italic text-muted-foreground">Select world</span>
        )}
