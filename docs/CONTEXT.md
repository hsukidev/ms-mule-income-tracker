...
- "Snapshot" is polysemous. The drag-to-select snapshot (per-mule **Active Flag** captured at gesture start) and the data-import snapshot (localStorage reads captured before write) are unrelated. Always qualify.
