...
> **Domain expert:** "No — that'd be a paintbrush, not what we want. Use **Drag-to-Select**. The **Start Card** fixes the **Brush** polarity for the gesture, and the **Paint Range** is recomputed on every `pointermove` from the **Start Card's** index to whatever card is under the pointer right now. If the user drags forward three cards, those three join the range and take the **Brush**. If they then drag back to the **Start Card**, those three fall *out* of the range and revert to their **Original Snapshot**."
